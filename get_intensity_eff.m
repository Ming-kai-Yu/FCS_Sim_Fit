function intensity_efficient =  get_intensity_eff(dt, nt, D, br, bl, ... 
		npart, r, l, m)
...
for j = 2:nt
    % update the locations of the particles inside the important region
    x_dat(inside_status,:) = x_dat(inside_status,:) + ...
        sigma_in * randn(npart_in, 3);
    rho_dat = x_dat(:,1).^2/r2 + x_dat(:,2).^2/r2 + x_dat(:,3).^2/l2;
    intensity_efficient(j) = sum(exp(-2*rho_dat(inside_status)));
    
    if mod(j, m) == 1
        % update the locations of the particles that were outside 
        %outside_status = 1- inside_status;
        %npart_out = sum(outside_status);
        x_dat(~inside_status,:) = x_dat(~inside_status,:) + ...
            sigma_out * randn(npart - npart_in, 3);
